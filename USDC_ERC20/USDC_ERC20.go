...
func Start(order sdb.Orders) bool {
	...
	if etherscanResp.Result[0].To == order.Token && timeStampMs > order.StartTime && timeStampMs < order.ExpirationTime && amount == order.ActualAmount && etherscanResp.Result[0].Hash != "" && etherscanResp.Result[0].TokenSymbol == "USDC" {
