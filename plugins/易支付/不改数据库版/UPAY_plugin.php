...
class UPAY_plugin {
    ...
    /**
     * 将数字编号转换为API格式
     * @param int $typeNumber 支付类型数字编号
     * @return string 返回API格式的支付类型字符串
     * @throws Exception 不支持的支付类型时抛出异常
     */
    static private function convertToApiType($typeNumber) {
        $typeMapping = [
            1 => 'USDT-TRC20',
            2 => 'TRX',
            3 => 'USDT-Polygon',
        ...
        if (isset($typeMapping[$typeNumber])) {
            return $typeMapping[$typeNumber];
        }
        
        throw new Exception('不支持的支付类型编号: ' . $typeNumber);
    }
    ...
    static private function CreateOrder() {
        ...
        $param = [
            ...
            'type' => self::convertToApiType($order['typename']), // 支付币种类型（数字编号转换为API格式）
