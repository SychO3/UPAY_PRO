...
/**
 * WHMCS UPAY Payment Gateway Module for USDT-ArbitrumOne
 * 
 * This module allows WHMCS to integrate with UPAY digital currency payment system.
 * It is specifically for USDT-ArbitrumOne payments.
 */
 ...
 */
function upay_usdt_arbitrumone_MetaData()
{
    return array(
        'DisplayName' => 'USDT-ArbitrumOne',
        'APIVersion' => '1.1',
 ...
 */
function upay_usdt_arbitrumone_config()
{
    return array(
        'FriendlyName' => array(
            'Type' => 'System',
            'Value' => 'USDT-ArbitrumOne',
        ),
        'apiurl' => array(
 ...
 */
function upay_usdt_arbitrumone_link($params)
{
    $apiUrl = rtrim($params['apiurl'], '/');
    $apiKey = $params['apikey'];
    $paymentType = 'USDT-ArbitrumOne'; // 固定支付类型
    ...
    $order = array(
        ...
        'type' => $paymentType,
        'notify_url' => $systemUrl . 'modules/gateways/callback/upay_usdt_arbitrumone.php',
        'redirect_url' => $returnUrl,
    );
    
    // Generate signature
    $signature = upay_usdt_arbitrumone_generateSignature($order, $apiKey);
    $order['signature'] = $signature;
    
    // Log request data for debugging
    logModuleCall(
        'upay_usdt_arbitrumone',
 ...
 */
function upay_usdt_arbitrumone_generateSignature($params, $apiKey)
{
    // 根据API文档要求，先提取所有参数键名
