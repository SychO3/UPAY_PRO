...
/**
 * WHMCS UPAY Payment Gateway Module
 * 
 * This module allows WHMCS to integrate with UPAY digital currency payment system.
 * It supports multiple cryptocurrency payment methods.
 */
 ...
 */
function upay_MetaData()
{
    return array(
        'DisplayName' => 'UPAY数字货币支付',
        'APIVersion' => '1.1',
...
function upay_link($params)
{
    ...
    $order = array(
        ...
        'type' => $paymentType,
        'notify_url' => $systemUrl . 'modules/gateways/callback/upay.php',
        'redirect_url' => $returnUrl,
    );
    
    // Generate signature
    $signature = upay_generateSignature($order, $apiKey);
    ...
    if ($httpCode !== 200) {
        ...
        if ($httpCode == 400) {
            ...
            return '<div class="alert alert-danger">支付接口请求失败，HTTP状态码：' . $httpCode . ' (' . $errorMsg . ')。请检查API密钥、支付类型和请求参数是否正确。</div>';
        } else {
 ...
 */
function upay_generateSignature($params, $apiKey)
{
    // 根据API文档要求，先提取所有参数键名
