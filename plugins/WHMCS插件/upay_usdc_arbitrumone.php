...
/**
 * WHMCS UPAY Payment Gateway Module for USDC-ArbitrumOne
 * 
 * This module allows WHMCS to integrate with UPAY digital currency payment system.
 * It is specifically for USDC-ArbitrumOne payments.
 */
 ...
 */
function upay_usdc_arbitrumone_MetaData()
{
    return array(
        'DisplayName' => 'USDC-ArbitrumOne',
        'APIVersion' => '1.1',
 ...
 */
function upay_usdc_arbitrumone_config()
{
    return array(
        'FriendlyName' => array(
            'Type' => 'System',
            'Value' => 'USDC-ArbitrumOne',
        ),
        'apiurl' => array(
 ...
 */
function upay_usdc_arbitrumone_link($params)
{
    $apiUrl = rtrim($params['apiurl'], '/');
    $apiKey = $params['apikey'];
    $paymentType = 'USDC-ArbitrumOne'; // 固定支付类型
    ...
    $order = array(
        ...
        'type' => $paymentType,
        'notify_url' => $systemUrl . 'modules/gateways/callback/upay_usdc_arbitrumone.php',
        'redirect_url' => $returnUrl,
    );
    
    // Generate signature
    $signature = upay_usdc_arbitrumone_generateSignature($order, $apiKey);
    $order['signature'] = $signature;
    
    // Log request data for debugging
    logModuleCall(
        'upay_usdc_arbitrumone',
 ...
 */
function upay_usdc_arbitrumone_generateSignature($params, $apiKey)
{
    // 根据API文档要求，先提取所有参数键名
