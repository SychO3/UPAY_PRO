...
/**
 * WHMCS UPAY Payment Gateway Module for USDT-Polygon
 * 
 * This module allows WHMCS to integrate with UPAY digital currency payment system.
 * It is specifically for USDT-Polygon payments.
 */
 ...
 */
function upay_usdt_polygon_MetaData()
{
    return array(
 ...
 */
function upay_usdt_polygon_config()
{
    return array(
        'FriendlyName' => array(
            'Type' => 'System',
 ...
 */
function upay_usdt_polygon_link($params)
{
    $apiUrl = rtrim($params['apiurl'], '/');
    $apiKey = $params['apikey'];
    $paymentType = 'USDT-Polygon'; // 固定支付类型
    ...
    $order = array(
        ...
        'type' => $paymentType,
        'notify_url' => $systemUrl . 'modules/gateways/callback/upay_usdt_polygon.php',
        'redirect_url' => $returnUrl,
    );
    
    // Generate signature
    $signature = upay_usdt_polygon_generateSignature($order, $apiKey);
 ...
 */
function upay_usdt_polygon_generateSignature($params, $apiKey)
{
    // 根据API文档要求，先提取所有参数键名
