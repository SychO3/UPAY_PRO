...
/**
 * WHMCS UPAY Payment Gateway Module for USDC-ERC20
 * 
 * This module allows WHMCS to integrate with UPAY digital currency payment system.
 * It is specifically for USDC-ERC20 payments.
 */
 ...
 */
function upay_usdc_erc20_MetaData()
{
    return array(
 ...
 */
function upay_usdc_erc20_config()
{
    return array(
        'FriendlyName' => array(
            'Type' => 'System',
 ...
 */
function upay_usdc_erc20_link($params)
{
    $apiUrl = rtrim($params['apiurl'], '/');
    $apiKey = $params['apikey'];
    $paymentType = 'USDC-ERC20'; // 固定支付类型
    ...
    $order = array(
        ...
        'type' => $paymentType,
        'notify_url' => $systemUrl . 'modules/gateways/callback/upay_usdc_erc20.php',
        'redirect_url' => $returnUrl,
    );
    
    // Generate signature
    $signature = upay_usdc_erc20_generateSignature($order, $apiKey);
 ...
 */
function upay_usdc_erc20_generateSignature($params, $apiKey)
{
    // 根据API文档要求，先提取所有参数键名
