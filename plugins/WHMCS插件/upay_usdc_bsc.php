...
/**
 * WHMCS UPAY Payment Gateway Module for USDC-BSC
 * 
 * This module allows WHMCS to integrate with UPAY digital currency payment system.
 * It is specifically for USDC-BSC payments.
 */
 ...
 */
function upay_usdc_bsc_MetaData()
{
    return array(
 ...
 */
function upay_usdc_bsc_link($params)
{
    $apiUrl = rtrim($params['apiurl'], '/');
    $apiKey = $params['apikey'];
    $paymentType = 'USDC-BSC'; // 固定支付类型
    ...
    $order = array(
        ...
        'type' => $paymentType,
        'notify_url' => $systemUrl . 'modules/gateways/callback/upay_usdc_bsc.php',
        'redirect_url' => $returnUrl,
    );
    
    // Generate signature
    $signature = upay_usdc_bsc_generateSignature($order, $apiKey);
 ...
 */
function upay_usdc_bsc_generateSignature($params, $apiKey)
{
    // 根据API文档要求，先提取所有参数键名
