 ...
 */
function upay_trx_link($params)
{
    $apiUrl = rtrim($params['apiurl'], '/');
    $apiKey = $params['apikey'];
    $paymentType = 'TRX'; // 固定支付类型
    ...
    $order = array(
        ...
        'type' => $paymentType,
        'notify_url' => $systemUrl . 'modules/gateways/callback/upay_trx.php',
        'redirect_url' => $returnUrl,
    );
    
    // Generate signature
    $signature = upay_trx_generateSignature($order, $apiKey);
 ...
 */
function upay_trx_generateSignature($params, $apiKey)
{
    // 根据API文档要求，先提取所有参数键名
